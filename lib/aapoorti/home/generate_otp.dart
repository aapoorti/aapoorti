...
class _GenerateOtpScreenState extends State<GenerateOtpScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Generate OTP', style: TextStyle(color: Colors.white)),
      ),
      body: Center(
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 20),
              const Text(
                'This option will work for foreign bidders only.',
                textAlign: TextAlign.center,
                style: TextStyle(
                  fontSize: 13,
    ...
    if(Platform.isIOS){
      ...
      await placemarkFromCoordinates(position.latitude, position.longitude).then((List<Placemark> placemarks) {
        ...
        if(place.country!.isNotEmpty) {
          ...
          if(place.country!.toLowerCase().trim().contains("india")) {
            debugPrint("Country ${place.country!}");
            //androidInfo.id == "UKQ1.230924.001" || androidInfo.id == "RP1A.200720.011" || androidInfo.id == "TP1A.220624.014"
            getGpsList().then((value){
                 debugPrint("devices id $value");
                 if(value.contains(androidInfo.id)){
                   String? code = getCountryCode(place.country!);
  ...
  Future<List<String>> getGpsList() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? jsonString = prefs.getString('gpsList');  // Get saved string
    if (jsonString != null) {
      List<dynamic> jsonResponse = jsonDecode(jsonString);  // Decode JSON string to List
  ...
  Future<void> _callLoginWebService(String phoneNumber, String countryCode, String uniqueId) async {
    try {
      ...
      pr!.hide();
      showDialog(context: context, barrierDismissible: false, builder: (ctx) => AlertDialog(
        title: const Text("Login Credential for IREPS Desktop Web Application", style: TextStyle(fontSize: 18)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
