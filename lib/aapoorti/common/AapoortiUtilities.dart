...
class AapoortiUtilities {
  ...
  static Widget attachDocsListView(BuildContext context, String attachDocsString) {
    ...
    return ListView.separated(
        itemCount: attachDocsString.length != 0 ? attachDocsArray.length : 0,
  ...
  static Widget corrigendumListView(BuildContext context, String attachCorriString) {
    ...
    debugPrint("Corrie array ${corriArray}");
    return ListView.separated(
        itemCount: attachCorriString.length != 0 ? corriArray.length : 0,
        itemBuilder: (context, index) {
          var corriURL = corriArray[index].split('#')[2].toString();
          debugPrint("Corrie URL ${corriURL}");
          return Container(
  ...
  static Future<void> pdfDialog(BuildContext context, String fileUrl, String fileName) async{
    if(Platform.isAndroid){
      return showDialog(
            context: context,
  ...
  static void downloadpdf(String fileurl, String filename, BuildContext context) async {
    ...
    downloadFile(fileurl, context).then((filepath) {
      if(progressDialog.isShowing()) {
        progressDialog.hide(); // Hide progress dialog if an error occurs
      }
  ...
  static Widget customTextView(String displayString, Color displayColor) {
    return Text( displayString.length != 0 ? displayString : "",
        style: TextStyle(
          color: displayColor,
          fontSize: 15,
        ),
