...
class _ValuewiseStockfilterState extends State<ValueWiseStockFilter> {
  ...
  Future<void> getInitData() async{
    ...
    if(providedTime.isBefore(DateTime.now())) {
      ...
      await Provider.of<ValueWiseProvider>(context, listen: false).getUnitTypeData("", context);
      await Provider.of<ValueWiseProvider>(context, listen: false).getUnitNameData("","", context);
      await Provider.of<ValueWiseProvider>(context, listen: false).getDepartment(context);
      await Provider.of<ValueWiseProvider>(context, listen: false).getConsignee("","","","", context);
      await Provider.of<ValueWiseProvider>(context, listen: false).getSubConsignee("","","","", "", context);
    }
    else {
      await Provider.of<ValueWiseProvider>(context, listen: false).getRailwaylistData(context);
      await Provider.of<ValueWiseProvider>(context, listen: false).getUnitTypeData("", context);
                      ...
                      children: [
                        ...
                        Consumer<ValueWiseProvider>(builder: (context, provider, child){
                          if(provider.defaultDataState == DefaultDataState.Busy) {
                            ...
                            return buildSearchableDropdown(
                                "Unit Name",
                                provider.unitname,
                                provider.unitnamelistData,
                                    (value) => provider.unitname = value,
                                Icons.location_city);
                          }
                        }),
                        const SizedBox(height: 16),
                        Consumer<ValueWiseProvider>(builder: (context, provider, child){
                          if(provider.defaultDataState == DefaultDataState.Busy) {
                            ...
                            return buildSearchableDropdown(
                                "Department",
                                provider.department,
                                provider.departmentlistData,
                                    (value) => provider.department = value,
                                Icons.work);
                          }
                        }),
                        const SizedBox(height: 16),
                        Consumer<ValueWiseProvider>(builder: (context, provider, child){
                          if(provider.defaultDataState == DefaultDataState.Busy) {
                            ...
                            return buildSearchableDropdown(
                                "User Depot",
                                provider.consignee,
                                provider.consigneelistData,
                                    (value) => provider.consignee = value,
                                Icons.warehouse);
                          }
                        }),
                        const SizedBox(height: 16),
                        Consumer<ValueWiseProvider>(builder: (context, provider, child){
                          if(provider.defaultDataState == DefaultDataState.Busy) {
                            ...
                            return buildSearchableDropdown(
                                "User Sub Depot",
                                provider.subconsignee,
                                provider.dropdowndata_UDMUserSubDepot,
                                    (value) => provider.subconsignee = value,
                                Icons.store);
                          }
                        }),
                        const SizedBox(height: 16),
            ...
            child: Row(
              children: [
                ...
                Consumer<ValueWiseProvider>(builder: (context, provider, child){
                   return Expanded(
                     child: Text(
                       label == "Railway" ? provider.railway : label == "Unit Type" ? provider.unittype : label == "Unit Name" ? provider.unitname : label == "Department" ? provider.department : label == "User Depot" ? provider.consignee : provider.subconsignee,
...
class _SearchableDropdownModalState extends State<SearchableDropdownModal> {
  ...
  void initState() {
    ...
    debugPrint("filter options ${widget.options}  $_filteredOptions.");
  }
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  onTap: () {
                    ...
                    widget.title == "Railway" ? Provider.of<ValueWiseProvider>(context, listen: false).railway = option['value']
                        : widget.title == "Unit Type" ? Provider.of<ValueWiseProvider>(context, listen: false).unittype = option['value'] :
                          widget.title == "Unit Name" ? Provider.of<ValueWiseProvider>(context, listen: false).unitname = option['value'] :
                          widget.title == "Department" ? Provider.of<ValueWiseProvider>(context, listen: false).department = option['value'] :
                          widget.title == "User Depot" ? Provider.of<ValueWiseProvider>(context, listen: false).consignee = option['value'] :Provider.of<ValueWiseProvider>(context, listen: false).subconsignee = option['value'];
                    Provider.of<ValueWiseProvider>(context, listen: false).setDefaultDataState(DefaultDataState.Finished);
                    Navigator.pop(context);
                  },
                );
              },
