...
class ConsumtionAnalysisFilter extends StatefulWidget {
  ...
  @override
  State<ConsumtionAnalysisFilter> createState() =>
      _ConsumtionAnalysisFilterState();
}
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(language.text('consAnalysis')),
        actions: [
          IconButton(
                          ...
                          decoratorProps: DropDownDecoratorProps(
                              decoration: InputDecoration(
                                  enabledBorder: OutlineInputBorder(
                                      ...
                                      borderSide: BorderSide.none),
                                  contentPadding: EdgeInsets.only(left: 10))),
                          items: (filter, loadProps) => itemTypeList.map((e) {
                            return e['value'].toString().trim();
                          }).toList(),
                          onChanged: (newValue) {
                          ...
                          decoratorProps: DropDownDecoratorProps(
                              decoration: InputDecoration(
                                  enabledBorder: OutlineInputBorder(
                                      ...
                                      borderSide: BorderSide.none),
                                  contentPadding: EdgeInsets.only(left: 10))),
                          items: (filter, loadProps) => itemUsageList.map((e) {
                            return e['value'].toString().trim();
                          }).toList(),
                          onChanged: (newValue) {
                          ...
                          onPressed: () {
                            setState(() {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content:
                                      const Text('All fields have been reset'),
                        ...
                        child: _buildActionButton(
                          ...
                          onPressed: () {
                            setState(() {
                                if (userDepotValue.toString() == "All" || userDepotValue.toString() == "-1") {
                                  itemListProvider =
                                      Provider.of<ConsAnalysisProvider>(context,
                                          listen: false);
                                  Navigator.of(context)
                                      .pushNamed(ConsAnalysisScreen.routeName);
                                  String pFrom = previousFromDateController.text.trim();
                                  String pTo = previousToDateController.text.trim();

                                  String cFrom = currentFromDateController.text.trim();
                                  String cTo = currentToDateController.text.trim();
                                  String percValue = '';
                                  if (!_isIncrease) {
                                    percValue = '2';
                                  } else {
                                  ...
                                  itemListProvider
                                      .fetchAndStoreItemsListwithdata(
                                          railway,
                                          department,
                                          "-1",
                                          //_formKey.currentState!.fields['User Depot']!.value,
                                  ...
                                  itemListProvider = Provider.of<ConsAnalysisProvider>(context, listen: false);
                                  Navigator.of(context).pushNamed(ConsAnalysisScreen.routeName);
                                  var depot = userDepotValue.split('-');
                                  String pFrom = previousFromDateController.text.trim();
                                  String pTo = previousToDateController.text.trim();
