...
//
// class NonMovingFilter extends StatefulWidget {
//   static const routeName = "/non-moving-filter";
//   @override
//   _NonMovingFilterState createState() => _NonMovingFilterState();
// }
//
// class _NonMovingFilterState extends State<NonMovingFilter> {
//   double? sheetLeft;
//   bool isExpanded = true;
//   late NonMovingProvider itemListProvider;
//   String? railway;
//   String? unittype;
//   String? department;
//   String? userDepot;
...
//     radioData = language.text('itemNotIssuedSince1Month');
...
//                   width: 10,
//                 ),
//                 Text(language.text('months')),
//               ],
//             ),
//             Align(
//               alignment: Alignment.centerLeft,
//               child: Text(radioData),
//             ),
//             SizedBox(height: 10),
//             Wrap(
//               spacing: 7.5,
...
//                              itemListProvider = Provider.of<NonMovingProvider>(context, listen: false);
//                              Navigator.of(context).pushNamed(NonMovingScreen.routeName);
...
class NonMovingFilter extends StatefulWidget {
  ...
  @override
  State<NonMovingFilter> createState() => _NonMovingFilterState();
}

class _NonMovingFilterState extends State<NonMovingFilter> with SingleTickerProviderStateMixin {
  ...
  Future<dynamic> default_data() async {
    ...
    try {
      ...
      setState(() {
        ...
        userDepotName = "${d_Json[0]['ccode'].toString()}-${d_Json[0]['cname'].toString()}";
        userDepotCode = d_Json[0]['ccode'].toString();
        userSubDepotCode = d_Json[0]['sub_cons_code'].toString();
        userSubDepotName = "${d_Json[0]['sub_cons_code'].toString()}-${d_Json[0]['sub_user_depot'].toString()}";
        Future.delayed(Duration(milliseconds: 0), () async {
          def_fetchUnit(
              d_Json[0]['org_zone'],
              d_Json[0]['org_unit_type'].toString(),
        ...
        if (UDMUserDepot_body['status'] != 'OK') {
          ...
          setState(() {
            ...
            if (depot_id != "") {
              ...
              dropdowndata_UDMUserDepot.forEach((item) {
                if (item['intcode'].toString().contains(depot_id.toLowerCase())) {
                  userDepotName = item['intcode'].toString() + '-' + item['value'];
                }
              });
              def_fetchSubDepot(rai, depot_id, userSubDep);
            } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
            Provider.of<LanguageProvider>(context).text('nonMovingItems'),
                        ...
                        child: _buildDropdownField(
                          ...
                          onChanged: (value) {
                            dropdowndata_UDMRlyList.forEach((element) {
                              if(element['value'].toString().trim() == value.toString().trim()) {
                                setState(() {
                                  railwayName = value!;
                                  railwayCode = element['intcode'].toString().trim();
                          ...
                          onChanged: (value) {
                            dropdowndata_UDMUnitType.forEach((element) {
                              if (element['value'].toString().trim() == value.toString().trim()) {
                                setState(() {
                                  unittypeName = value!;
                                  unittypeCode = element['intcode'].toString().trim();
                                });
                              }
                            });
                      ...
                      Expanded(
                        child: _buildDropdownField(
                          ...
                          onChanged: (value) {
                            ...
                            def_fetchDepot(railwayCode, deptCode, unittypeCode, unitnameCode, '', '');
                          },
                        ),
                      ),
                    ],
                  ...
                  _buildDropdownField(
                    ...
                    onChanged: (value) {
                      dropdowndata_UDMUserSubDepot.forEach((element) {
                        if("${element['intcode'].toString().trim()}-${element['value'].toString().trim()}" == value.toString().trim()) {
                          setState(() {
                  ...
                  Expanded(
                    child: _buildPrimaryButton(
                      ...
                      onPressed: () {
                        ...
                        if(userDepotName.toString() == "All" || userDepotCode.toString() == "-1" || userDepotName == null) {
                          itemListProvider = Provider.of<NonMovingProvider>(context, listen: false);
                          ...
                          Navigator.of(context).pushNamed(NonMovingScreen.routeName, arguments: [railwayCode,
                            unittypeCode,
                            unitnameCode,
                            deptCode,
                            userDepotCode,
                          ...
                          String? itemUsge,itemUnit, itemCat,sns;
                          itemListProvider = Provider.of<NonMovingProvider>(context, listen: false);
                          if (!_expandedFilters['Item Usage']!) {
                            itemUsge = '-1';
                          } else {
