...
class VersionProvider with ChangeNotifier {
  ...
  Future<void> fetchVersion(BuildContext context) async {
    try {
      ...
      if(json.decode(response.body)['status'] == "OK" && json.decode(response.body)['message'] == "Success!"){
        ...
        List<String> gpsbypasslist = versionResult['GpsByPassDevices']!.split('#');
        savegpslist(gpsbypasslist);
  ...
  Future<void> savegpslist(List<String> _myList) async{
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String jsonString = jsonEncode(_myList);  // Convert list to JSON string
    await prefs.setString('gpsList', jsonString);  // Save to SharedPreferences
    debugPrint('GPS List saved!');
  }
